...
import { CreditCard, Calendar, FileText, Package, Server, IndianRupee } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function ExpenseLedger({ ledgerData, loading, selectedExpense, expenses, dateRange }: ExpenseLedgerProps) {
  ...
  const safeAmount = (n?: number) => Math.abs(Number(n || 0));
    ...
    return (
      <div className="space-y-4 sm:space-y-6">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {[1, 2, 3, 4].map((i) => (
            <Card key={i} className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border-slate-200 dark:border-slate-700 shadow-sm">
          ...
          <CardHeader className="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 border-b border-green-200 dark:border-green-800/50 p-3 sm:p-6">
            <CardTitle className="text-green-700 dark:text-green-400 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-base sm:text-lg">Expense Payments</span>
              </div>
        ...
        <CardContent className="p-0">
          {allCreditEntries.length === 0 ? (
            <div className="text-center py-8 sm:py-12 text-slate-500 dark:text-slate-400 px-4">
              ...
              <p className="text-xs mt-2">Cash expenses and payment entries will appear here</p>
            </div>
          ) : (
            <>
              {/* Mobile Card View */}
              ...
              <div className="hidden lg:block max-h-96 overflow-y-auto custom-scrollbar">
                <table className="w-full border-collapse">
                  <thead className="sticky top-0 bg-green-100 dark:bg-green-900/40 z-10">
