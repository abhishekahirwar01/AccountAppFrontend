...
export class CompanyFormValidations {
  // Mobile number validation (Indian format)
  static validateMobileNumber(mobile: string): ValidationResult {
  ...
  static validatePANNumber(pan: string): ValidationResult {
    if (!pan || pan.trim() === '') {
      return { isValid: true, message: '' }; // Empty is allowed
    }
...
static validateGSTIN(gstin: string): ValidationResult {
  if (!gstin || gstin.trim() === '') {
    return { isValid: true, message: '' };
  }
  ...
  if (!gstinRegex.test(cleanedGSTIN)) {
    return { 
      isValid: false, 
      message: 'Please enter a valid GSTIN (15 characters: 2-digit state code + 10-char business ID + entity + Z + checksum)' 
    };
  }
  ...
  static validateTANNumber(tan: string): ValidationResult {
    if (!tan || tan.trim() === '') {
      return { isValid: true, message: '' }; // Empty is allowed
    }
  ...
  static validateFormFields(fields: {
    ...
    if (fields.mobileNumber !== undefined) {
      results.mobileNumber = this.validateMobileNumber(fields.mobileNumber);
    }

    if (fields.emailId !== undefined) {
      results.emailId = this.validateEmail(fields.emailId);
    }

    if (fields.PANNumber !== undefined) {
      results.PANNumber = this.validatePANNumber(fields.PANNumber);
    }

    if (fields.gstin !== undefined) {
      results.gstin = this.validateGSTIN(fields.gstin);
    }

    if (fields.TANNumber !== undefined) {
      results.TANNumber = this.validateTANNumber(fields.TANNumber);
    }
...
export const useCompanyFormValidation = () => {
  ...
  const validateMultipleFields = (fields: Record<string, string>) => {
